...
export async function generateCountReport ({ user, start, end }: reportArgs): Promise<string> {
...
export async function generateXlsxReport ({ user, start, end }: reportArgs): Promise<ExcelJS.Workbook> {
  ...
  const rows = tasks.map(e => 
    [e.numericId, e.title, e.hours.toLocaleString('ru-RU') + 'ч']
  )
