...
export async function selectDateSpan (ctx: BotSceneContext): Promise<void> {
  ...
  if (ctx.scene.step.firstTime || ctx.payload.data === BotPayload.CloseCalendar) {
    ctx.scene.state.dateStart = ''
    ctx.scene.state.dateEnd = ''
    const keyboard = new InlineKeyboardBuilder()
      .textButton({ text: 'Текущий месяц', payload: BotPayload.DatespanCurrentMonth }).row()
      .textButton({ text: 'Прошлый месяц', payload: BotPayload.DatespanPrevMonth }).row()
      .textButton({ text: 'Выбрать в календаре', payload: BotPayload.ShowCalendar + ':' + currentMonth }).row()
  ...
  switch (key) {
    ...
    case BotPayload.ShowCalendar: {
      ...
      calKeyboard.textButton({ text: 'Назад', payload: BotPayload.CloseCalendar })
      ...
      if (ctx.scene.state.dateStart) {
        ...
        calKeyboard.textButton({ text: 'Изм. дату начала', payload: BotPayload.RemoveDateStart + ':' + currentMonth })
      }
    ...
    case BotPayload.SelectCalendarDate: {
      if (ctx.scene.state.dateStart) {
        ...
        await ctx.editText(`[2/3] 📅 Выберите даты:\nВыбрана дата начала: ${dateHuman}`, { reply_markup: calKeyboard })
        return
      }
    }
...
function inlineCalendar (year: number, month: number): InlineKeyboardBuilder {
  const currentMonth = dayjs().set('year', year).set('month', month).startOf('month')
  ...
  for (let d = dateStart; d.isBefore(dateEnd); d = d.add(1, 'day')) {
    ...
    if (d.isSame(currentMonth, 'month')) {
      keyboard.textButton({ text: d.format('D'), payload: `scal:${d.format('YYYY-MM-DD')}` })
    } else {
